...
for classURL in URL:
    ...
    if '光学' in returnText:
        for teacherName in jiaoshi_optics.keys():
            if teacherName in returnText:
                if (teacherName in part_name.keys()) and (part_name[teacherName] in returnText):
                    teacherName=part_name[teacherName]
                group = jiaoshi_optics[teacherName]
    else:
        for teacherName in jiaoshi_modern.keys():
            if teacherName in returnText:
                if (teacherName in part_name.keys()) and (part_name[teacherName] in returnText):
                    teacherName=part_name[teacherName]
                group = jiaoshi_modern[teacherName]
    # 创建一个worksheet
    ...
    if 'e=3' in nameStr:  # 把T_改为光学的下标 T stands for termal L stands for optics
        nameStrNew = 'L_' + \
            nameStr[nameStr.index(
                '2021-')+5:nameStr.index('2021-')+10] + '_C'+str(int(nameStr[-1])+1)
    else:  # 把E_改为近代的下标 E stands for electromagnetic M stands for modern physics
        nameStrNew = 'M_' + \
            nameStr[nameStr.index(
                '2021-')+5:nameStr.index('2021-')+10] + '_C'+str(int(nameStr[-1])+1)
